...
from plot import output_csv

# Visdom
port = 8097
train_loss_logger = VisdomPlotLogger('line', port=port, opts={'title': 'Train Loss'})
val_epoch_loss_logger = VisdomPlotLogger('line', port=port, opts={'title': 'Val Epoch Loss'})
val_aa_logger = VisdomPlotLogger('line', port=port, opts={'title': 'AA'})
val_miou_logger = VisdomPlotLogger('line', port=port, opts={'title': 'mIOU'})
...
class ModelTrainer:
    ...
    def train(self, model, train_loader, val_loader, loss_fn, optimizer, step=0):
        ...
        for i, batch in enumerate(train_loader):
            ...
            # print(global_step, loss.cpu().detach().numpy())
            train_loss_logger.log(global_step+1, loss.cpu().detach().numpy())

            # run validation
            if global_step > 0 and global_step % self.args.val_freq == 0:
                self.val(model, val_loader, global_step, loss_fn)
    ...
    def val(self, model, dataloader, step, loss_fn):
        ...
        val_epoch_loss_logger.log(step, loss / len(dataloader))
        val_aa_logger.log(step, conf_mat.get_aa())
        val_miou_logger.log(step, conf_mat.get_mIoU())

        # Print matrix
        output_csv(conf_mat.state)
...
def main():
    ...
    # train dfc
    parser.add_argument('--train_with_dfc', action='store_true', default=False,
                        help='train with dfc dataset')
    ...
    trainer.export_model(model, optimizer=optimizer, name="final")
